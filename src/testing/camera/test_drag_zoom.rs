...
fn test_drag_zoom() {
    ...
    for alt in altitudes {
        ...
        let start_pos = Vec3::new(
            0.0,
            stuttgart_surface_y as f32,
            stuttgart_surface_z as f32 + alt as f32,
        );
        ...
        let paths = vec![(
            "Drag Across Screen (Ellipsoid Parallax Lag Check)",
            (960.0, 540.0),
            (1460.0, 540.0),
            50,
        )];
        ...
        for (name, start_px, end_px, steps) in paths {
            ...
            let (ray_origin, ray_dir) =
                cam.screen_to_world_ray(start_x, start_y, screen_w, screen_h);
            ...
            let (new_ray_o, new_ray_d) =
                cam.screen_to_world_ray(final_x, final_y, screen_w, screen_h);
            ...
            if let Some(new_hit) = intersect_ellipsoid(new_ray_o, new_ray_d) {
                ...
                println!(
                    "    Earth Point Drift on ELLIPSOID: {:.6} meters",
                    drift_meters
                );
