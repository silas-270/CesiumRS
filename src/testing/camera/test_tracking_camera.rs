...
mod tests {
    ...
    fn test_tracking_camera_orbit_mouse() {
        ...
        println!(
            "Camera length after pitch down: {} (Initial: {})",
            camera.local_pos.length(),
            initial_length
        );
        ...
        assert!(
            (camera.local_pos.length() - initial_length).abs() < 0.000001,
            "Camera distance changed due to orbit collision!"
        );
    }
    ...
    fn test_tracking_camera_zoom() {
        ...
        assert!(
            (camera.local_pos.z - 0.00002).abs() < 1e-7,
            "Expected camera to be clamped to 0.00002, got {:?}",
            camera.local_pos.z
        );
