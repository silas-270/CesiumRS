...
pub fn run_test() {
    ...
    for (i, (pos, pitch, yaw)) in test_cases.into_iter().enumerate() {
        ...
        camera.local_ori =
            glam::Quat::from_euler(glam::EulerRot::YXZ, yaw as f32, pitch as f32, 0.0);

        let aspect_ratio = 16.0 / 9.0;
        let frustum = camera.calculate_frustum_planes(aspect_ratio);
        let view_proj =
            camera.get_projection_matrix_f64(aspect_ratio as f64) * camera.get_view_matrix_f64();
        ...
        for lat_deg in (-85..=85).step_by(5) {
            for lon_deg in (-180..180).step_by(5) {
                ...
                if normal.dot(cam_to_point) > 0.0 {
                    continue;
                }
                    ...
                    && ndc.z <= 1.0
                {
                    ...
                    let y_float = (1.0
                        - (lat_rad.tan() + 1.0 / lat_rad.cos()).ln() / std::f32::consts::PI)
                        / 2.0
                        * (n as f32);
                    ...
                    if !is_tile_visible(&visible_tiles, id) {
                        ...
                        println!(
                            "Point Lat: {}, Lon: {} is ON SCREEN and front-facing.",
                            lat_deg, lon_deg
                        );
    ...
    if false_negatives_found > 0 {
        panic!(
            "Found {} false negatives in culling logic!",
