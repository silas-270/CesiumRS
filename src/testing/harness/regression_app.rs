...
impl<'a> RegressionApp<'a> {
    pub fn new(config: VerifyConfig) -> Self {
        ...
        let app_config = cesium_engine::globe::tiles::config::TileEngineConfig {
            offline_mode: true,
            mesh_cache_size: std::num::NonZeroUsize::new(10000).unwrap(),
            max_cache_size: std::num::NonZeroUsize::new(10000).unwrap(),
        ...
        if let WindowEvent::RedrawRequested = event {
            ...
            while self.current_state_index < self.states.len() {
                ...
                if let Some(state) = self.inner.wgpu_state_mut() {
                    ...
                    let _main_view_proj = state.camera.get_projection_matrix(aspect_ratio)
