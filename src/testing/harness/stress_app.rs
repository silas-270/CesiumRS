...
impl<'a> StressApp<'a> {
    pub fn new(config: VerifyConfig) -> Self {
        ...
        writeln!(
            log_file,
            "frame,speed_multiplier,requested_tiles,missing_tiles"
        )
        .unwrap();
        Self {
            inner: App::new(
    ...
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        if !self.setup_done {
            if let Some(state) = self.inner.wgpu_state_mut() {
                ...
                state
                    .camera
                    .set_eye(glam::Vec3::new(0.0, 0.0, 8.0), glam::Vec3::ZERO);
            }
        ...
        if let WindowEvent::RedrawRequested = event {
            ...
            if let Some(state) = self.inner.wgpu_state_mut() {
                if self.config.stress_mode == "poi" {
                    ...
                    state
                        .camera
                        .set_eye(glam::Vec3::new(eye_x, eye_y, eye_z), center);
                } else {
                    ...
                    state.camera.set_eye(
                        ...
                        glam::Vec3::new(look_x, look_y, 0.0),
                    );
                }
                ...
                match render_res {
                    Ok(_) => {
                        ...
                        writeln!(
                            self.log_file,
                            "{},{},{},{}",
                            self.frame_count, speed_multiplier, req, miss
                        )
                        .unwrap();
