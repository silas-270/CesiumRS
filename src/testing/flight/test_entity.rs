...
fn test_sampled_position_property() {
    ...
    // Exact samples
    assert_eq!(
        prop.evaluate(SimulationTime::new(0.0)).unwrap(),
        DVec3::new(0.0, 0.0, 0.0)
    );
    assert_eq!(
        prop.evaluate(SimulationTime::new(10.0)).unwrap(),
        DVec3::new(10.0, 0.0, 0.0)
    );

    // Midpoints
    assert_eq!(
        prop.evaluate(SimulationTime::new(5.0)).unwrap(),
        DVec3::new(5.0, 0.0, 0.0)
    );
    assert_eq!(
        prop.evaluate(SimulationTime::new(15.0)).unwrap(),
    ...
    assert_eq!(
        prop.evaluate(SimulationTime::new(25.0)).unwrap(),
...
fn test_adaptive_subdivision_stress() {
    ...
    prop.add_sample(
        SimulationTime::new(0.0),
        DVec3::from_array(cesium_engine::globe::geometry::lon_lat_alt_to_ecef_f64(
            ny_lon, ny_lat, 10000.0,
        )),
    );
    // Add midpoint to help the spline (flying over north pole roughly)
    prop.add_sample(
        SimulationTime::new(3600.0 * 8.0),
        DVec3::from_array(cesium_engine::globe::geometry::lon_lat_alt_to_ecef_f64(
            14.0, 85.0, 10000.0,
        )),
    );
    prop.add_sample(
        SimulationTime::new(3600.0 * 16.0),
        DVec3::from_array(cesium_engine::globe::geometry::lon_lat_alt_to_ecef_f64(
            sin_lon, sin_lat, 10000.0,
        )),
    );
