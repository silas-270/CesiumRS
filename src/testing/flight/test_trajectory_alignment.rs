...
fn test_plane_tangent_alignment() {
    ...
    let dt = 0.001;
    let pos_next = prop
        .evaluate(SimulationTime::new(time.seconds + dt))
        .unwrap();
    let pos_prev = prop
        .evaluate(SimulationTime::new(time.seconds - dt))
        .unwrap();
    ...
    assert!(
        distance < 1e-4,
        "Plane vector is not perfectly aligned with polyline tangent!"
    );
}
...
fn test_split_delta() {
    ...
    let check_at_time = |t: f64| {
        ...
        for i in 0..=100 {
            let pt_time = SimulationTime::new(t - 100.0 + i as f64 * 2.0)
                .seconds
                .max(0.0)
                .min(10000.0);
        ...
        for i in 0..verts.len() - 1 {
            ...
            let pos_curr1 = DVec3::new(
                v1.position[0] as f64,
                v1.position[1] as f64,
                v1.position[2] as f64,
            );
            let pos_prev1 = DVec3::new(
                v1.previous[0] as f64,
                v1.previous[1] as f64,
                v1.previous[2] as f64,
            );
            ...
            let pos_curr2 = DVec3::new(
                v2.position[0] as f64,
                v2.position[1] as f64,
                v2.position[2] as f64,
            );
            let pos_prev2 = DVec3::new(
                v2.previous[0] as f64,
                v2.previous[1] as f64,
                v2.previous[2] as f64,
            );
