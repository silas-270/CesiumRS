//! **The deep-detail pyramid**, photographed — the four levels below the source ceiling.
//!
//! The claim is one sentence: *below the z15 source ceiling the mesh is still a
//! decimation of data the engine already holds, and previously `DETAIL_MAX_Z = 15` reported that
...
//!
//! # The poses are approaches, not the earlier ten
//!
//! Every table before this one is measured on `real_poses`: forward views pitched 1-18°
//! below the horizontal from 300 m to 400 km, chosen for terrain occlusion, which is a question about the
//! far field. There is no near field in them — the settled tree at `alps_inn_valley`
//! bottoms out at z15 and stays there whatever the deep-detail pyramid does, which is exactly what the cost
...
//!
//! Every pose is rendered **four** times: `detail_max_z` at 15 and 19, each at
...
//! the near field visibly gains, while the z15 ceiling — which forbids any error below z15 —
...
/// `terrain_e1_capture::config` with the two knobs as the variables and everything else
/// held where that file holds it — terrain occlusion on, `mesh_segments` 16, Esri imagery.
...
/// axis — `terrain_e2_capture::differing_fraction`'s rule.
...
/// **not** "the deep-detail pixels are at the bottom". What the measured runs show is that the pixels
...
/// moves. That is the rule "the extra tiles land on the mountains", one ceiling deeper.
